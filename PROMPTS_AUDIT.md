...
Расширение работает как сборщик готовой карточки:

1. Пользователь выбирает одну или несколько тем, атмосфер и встреч/механик.
2. Модель создаёт новую AU-анкету `{{char}}`, сохраняя только узнаваемый характер, голос и поведенческую логику оригинала.
3. На основе уже адаптированной анкеты модель пишет стартовое приветствие.
4. Пользователь проверяет результат и может отредактировать приветствие в модальном окне.
5. Экспорт PNG/JSON доступен на финальном этапе и использует уже собранную карточку.
...
Если модель возвращает анкету без признаков яйцекладки или оставляет `{{char}}` обычным человеком без совместимой физиологии, результат отклоняется и не применяется к карточке.

## Адаптация анкеты

Функция: `buildAdaptCharacterArial()`.

Адаптация создаёт новую AU-анкету, а не бережно переписывает старую. Исходная анкета используется только как источник характера, голоса, эмоциональной логики, привычек, страхов, предпочтений и узнаваемых поведенческих особенностей `{{char}}`.

Промпт требует заменить или пересобрать под выбранную вселенную:

- мир и эпоху;
- сценарий;
- биографию;
- место проживания;
- роль в обществе или занятие;
- связи;
- цели и секреты;
- одежду;
- вид, тело и анатомию, если этого требует AU-механика;
- сексуальность и интимные особенности, если этого требует выбранная встреча.
