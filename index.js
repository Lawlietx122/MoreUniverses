...
function buildV2CardData(entry) {
  ...
  const userCreator = (typeof entry.creatorName === 'string' ? entry.creatorName.trim() : '')
    || (typeof extension_settings[extensionName]?.creatorName === 'string' ? extension_settings[extensionName].creatorName.trim() : '');
  const creatorField = userCreator || '';
...
function buildEntryFromContext(charName, storyText, themeBadge, themeId) {
  ...
  return {
    ...
    creatorName: extension_settings[extensionName].creatorName || '',
  };
}
...
// declared in style.css per #sea-of-universes-modal-overlay[data-theme="..."].
...
function applyHideDefault(hide) {
  if (hide) $('.au-opt-builtin').addClass('sou-hidden-builtin');
  else $('.au-opt-builtin').removeClass('sou-hidden-builtin');
}
...
function showQuickSettings() {
  ...
  const renderOption = (group, id, labelFn, selectedIds, builtinSet) => {
    ...
    return `<label class="au-multi-option${isBuiltin ? ' au-opt-builtin' : ''}"><input type="checkbox" class="au-multi-check" data-group="${group}" value="${id}" ${checked}> <span>${escapeHtml(labelFn(id))}</span></label>`;
  };
  ...
  const cardStructureRows = [
    ...
    'first_mes', 'alternate_greetings', 'tags (max 3)', 'creator', 'notes (always empty)',
  ];
  ...
  const html = `
    <div id="sea-of-universes-modal-overlay" style="${getOverlayStyle()}">
        <div class="au-universal-popup au-quick-popup sou-popup">
            ...
            <div class="sou-tabs">
                <button type="button" class="sou-tab sou-tab-active" data-tab="world">Сюжет и мир</button>
                <button type="button" class="sou-tab" data-tab="gen">Генерация</button>
                <button type="button" class="sou-tab" data-tab="templates">Шаблоны</button>
                <button type="button" class="sou-tab" data-tab="settings">Настройки</button>
            </div>
            <div class="au-universal-popup-body au-quick-body">
                ...
                <div class="sou-grid2">
                  <div class="sou-field">
                    ...
                    <div id="au-quick-custom-wrap" class="sou-inline-custom" style="${themeCustomDisplay}">
                      ...
                      <textarea id="au-quick-custom-theme" class="text_pole" rows="4" maxlength="${CUSTOM_THEME_MAX_LENGTH}" placeholder="${esc(t('quick.custom_theme_placeholder'))}" style="width:100%; resize:vertical; min-height:88px; margin-top:4px;">${esc(curCustomTheme)}</textarea>
                    </div>
                  </div>
                  <div class="sou-field">
              ...
              <div class="sou-tab-panel" data-panel="gen">
                ...
                <div class="sou-field" style="padding-top:8px; border-top:0.5px solid var(--border); margin-top:8px;">
                  <label style="font-weight:600;">Что генерировать</label>
                  <div class="sou-gen-flags">
                    <label class="sou-gen-flag"><input type="checkbox" id="au-quick-gen-personality" ${genP ? 'checked' : ''}/> <span class="sou-gen-name">Personality</span> <span class="sou-dot-marker" data-for="personality"></span> <button type="button" class="sou-btn-secondary sou-field-prompt-btn" data-field="personality">Свой промпт</button></label>
                    <label class="sou-gen-flag"><input type="checkbox" id="au-quick-gen-scenario" ${genS ? 'checked' : ''}/> <span class="sou-gen-name">Scenario</span> <span class="sou-dot-marker" data-for="scenario"></span> <button type="button" class="sou-btn-secondary sou-field-prompt-btn" data-field="scenario">Свой промпт</button></label>
                    <label class="sou-gen-flag"><input type="checkbox" id="au-quick-gen-mes" ${genM ? 'checked' : ''}/> <span class="sou-gen-name">Message Example</span> <span class="sou-dot-marker" data-for="mes_example"></span> <button type="button" class="sou-btn-secondary sou-field-prompt-btn" data-field="mes_example">Свой промпт</button></label>
                  </div>
              ...
              <div class="sou-tab-panel" data-panel="settings">
                <div class="sou-field" style="padding-top:4px;">
            ...
            <style>
              .sou-popup { max-width: 640px !important; background: var(--surface) !important; color: var(--text-primary) !important; }
              ...
              .sou-inline-custom { margin-top:8px; padding-top:8px; border-top:1px solid var(--border); display:flex; flex-direction:column; gap:4px; }
  ...
  function refreshCustomMarkers() {
    for (const [field, lsKey] of Object.entries(FIELD_LS)) {
      ...
      $(`.sou-dot-marker[data-for="${field}"]`).toggleClass('sou-dot-active', has);
    }
  }
  ...
  function refreshAntislopStatus() {
    ...
    $('.sou-save-status[data-for="antislop-card"]').toggleClass('sou-saved', !!cur1.trim()).text(cur1.trim() ? 'сохранено' : 'не сохранено');
    $('.sou-save-status[data-for="antislop-firstmes"]').toggleClass('sou-saved', !!cur2.trim()).text(cur2.trim() ? 'сохранено' : 'не сохранено');
  }
  ...
  const renderActiveTemplateSelect = () => {
    ...
    $('#au-quick-active-template').html(opts);
  };
  renderTemplatesList();
  ...
  const openTemplateEditor = (idx) => {
    ...
    $('#sou-template-name').val(tpl.name || '');
    $('#sou-template-body').val(tpl.template || '');
    $('#sou-template-antislop-card').val(tpl.antiSlopCard || '');
    $('#sou-template-antislop-firstmes').val(tpl.antiSlopFirstMes || '');
    $('#sou-template-editor').slideDown(120);
  };
  $('#sou-new-template').on('click', () => openTemplateEditor(-1));
  $('#sou-templates-list').on('click', '.sou-tpl-select', function () { applyTemplate(parseInt($(this).data('idx'), 10)); });
  $('#sou-templates-list').on('click', '.sou-tpl-edit', function () { openTemplateEditor(parseInt($(this).data('idx'), 10)); });
  $('#sou-templates-list').on('click', '.sou-tpl-del', function () {
  ...
  $('#sou-template-save').on('click', () => {
    const tpl = {
      name: String($('#sou-template-name').val() || '').trim().slice(0, 80) || `Шаблон ${getTemplates().length + 1}`,
      template: String($('#sou-template-body').val() || '').trim(),
      antiSlopCard: String($('#sou-template-antislop-card').val() || '').trim(),
      antiSlopFirstMes: String($('#sou-template-antislop-firstmes').val() || '').trim(),
    };
    const arr = getTemplates();
    ...
    renderTemplatesList();
    renderActiveTemplateSelect();
    $('#sou-template-editor').slideUp(120);
    toastr.success('Шаблон сохранён');
  });
  $('#sou-template-cancel').on('click', () => { $('#sou-template-editor').slideUp(120); editingTemplateIdx = -1; });
  $('#au-quick-manage-templates').on('click', () => {
    $('.sou-tab[data-tab="templates"]').trigger('click');
  });
  $('#au-quick-active-template').on('change', function () { applyTemplate(parseInt($(this).val(), 10)); });

  // === Tabs ===
  $('.sou-tab').on('click', function () {
    const tab = $(this).data('tab');
    $('.sou-tab').removeClass('sou-tab-active');
    $(this).addClass('sou-tab-active');
    $('.sou-tab-panel').removeClass('sou-tab-panel-active');
    $(`.sou-tab-panel[data-panel="${tab}"]`).addClass('sou-tab-panel-active');
  });
  ...
  const renderSavedRow = group => {
    ...
    const $row = $(`.au-saved-row[data-group="${group}"]`);
    if (!arr.length) { $row.html(`<div class="au-saved-empty">${escapeHtml(t('quick.no_saved_items'))}</div>`); return; }
    $row.html(arr.map(item => `<div class="au-saved-item" data-id="${item.id}"><span>${esc(item.label)}</span><span class="au-saved-del" data-group="${group}" data-id="${item.id}" title="${esc(t('quick.delete_saved'))}">🗑</span></div>`).join(''));
  };
  ['theme', 'encounter', 'mood', 'archetype', 'genre'].forEach(renderSavedRow);

  // === Search filtering ===
  $('.au-multi-search').on('input', function () {
    const group = $(this).data('group');
    const q = String($(this).val() || '').toLowerCase().trim();
    $(`.au-multi-check[data-group="${group}"]`).closest('.au-multi-option').each(function () {
  ...
  // === Delete saved items ===
  $(document).off('click.au-saved-del').on('click.au-saved-del', '.au-saved-del', function (e) {
    e.stopPropagation();
    const group = $(this).data('group');
  ...
  const rerenderOptions = group => {
    ...
    $(`.au-multi-select[data-group="${group}"] .au-multi-options`).html(opts);
    $(`#au-${group}-summary`).text(selectionSummary(selectedIds, labelFn));
    applyHideDefault(extension_settings[extensionName].hideDefaultOptions === true);
  };
  ...
  const syncMultiChecks = (group, obj, labelFn, settingKey, singleKey, customWrapSelector, fallback = 'random') => {
    const checks = $(`.au-multi-check[data-group="${group}"]`);
    ...
    $(`#au-${group}-summary`).text(selectionSummary(values, labelFn));
    if (customWrapSelector) { values.includes('custom') ? $(customWrapSelector).show() : $(customWrapSelector).hide(); }
    saveSettingsDebounced();
  };
  // Delegated handlers (survive rerenderOptions replacing the checkbox HTML).
  // Meta options (custom/random/none) auto-close the dropdown and toggle the
  // inline custom textarea via slideToggle.
  ...
  const handleMultiChange = (group, obj, labelFn, settingKey, singleKey, customWrapSelector, fallback) => (e) => {
    ...
    if (customWrapSelector) {
      ...
      if (values.includes('custom')) $(customWrapSelector).stop(true, true).slideDown(150);
      else $(customWrapSelector).stop(true, true).slideUp(150);
    }
  };
  ...
  $('#au-quick-custom-theme').on('input', function () { const v = sanitizeCustomTheme($(this).val()); extension_settings[extensionName].customTheme = v; saveSettingsDebounced(); $('#au-quick-custom-count').text(`${v.length}/${CUSTOM_THEME_MAX_LENGTH}`); });
  $('#au-quick-custom-encounter').on('input', function () { const v = sanitizeCustomEncounter($(this).val()); extension_settings[extensionName].customEncounter = v; saveSettingsDebounced(); $('#au-quick-custom-encounter-count').text(`${v.length}/${CUSTOM_ENCOUNTER_MAX_LENGTH}`); });
  $('#au-quick-custom-mood').on('input', function () { const v = sanitizeCustomMood($(this).val()); extension_settings[extensionName].customMood = v; saveSettingsDebounced(); $('#au-quick-custom-mood-count').text(`${v.length}/${CUSTOM_MOOD_MAX_LENGTH}`); });

  // === Hide default toggle ===
  $('#au-quick-hide-default').on('click', function () {
  ...
  $('#au-quick-story-length').on('change', function () { const v = $(this).val(); extension_settings[extensionName].storyLength = v; saveSettingsDebounced(); $('#au-quick-length-hint').toggle(v !== 'short' && v !== 'medium'); });
  $('#au-quick-first-mes-lang').on('change', function () { extension_settings[extensionName].firstMesLanguage = $(this).val(); saveSettingsDebounced(); });
  $('#au-quick-greeting-variants').on('change', function () { const v = Math.max(1, Math.min(3, parseInt($(this).val(), 10) || 1)); extension_settings[extensionName].greetingVariants = v; saveSettingsDebounced(); $('#au-quick-variant2').toggle(v >= 2); $('#au-quick-variant3').toggle(v >= 3); });
  $('#au-quick-adapt-language').on('change', function () { extension_settings[extensionName].adaptLanguage = $(this).val(); saveSettingsDebounced(); });
  $('#au-quick-adapt-size').on('change', function () { extension_settings[extensionName].adaptSize = $(this).val(); saveSettingsDebounced(); });
  $('#au-quick-adapt-custom-prompt').on('input', function () { extension_settings[extensionName].adaptCustomArial = String($(this).val() || '').trim(); saveSettingsDebounced(); });
  $('#au-quick-antislop-card').on('input', function () { extension_settings[extensionName].antiSlopCard = String($(this).val() || '').trim(); saveSettingsDebounced(); refreshAntislopStatus(); });
  $('#au-quick-antislop-firstmes').on('input', function () { extension_settings[extensionName].antiSlopFirstMes = String($(this).val() || '').trim(); saveSettingsDebounced(); refreshAntislopStatus(); });

  // === Generation flags ===
  $('#au-quick-gen-personality').on('change', function () { extension_settings[extensionName].genPersonality = $(this).prop('checked'); saveSettingsDebounced(); });
  $('#au-quick-gen-scenario').on('change', function () { extension_settings[extensionName].genScenario = $(this).prop('checked'); saveSettingsDebounced(); });
  $('#au-quick-gen-mes').on('change', function () { extension_settings[extensionName].genMesExample = $(this).prop('checked'); saveSettingsDebounced(); });
  ...
  $('.sou-field-prompt-btn').on('click', function () {
    const field = String($(this).data('field') || '');
    $('#sou-field-prompt-editor').attr('data-active-field', field);
    $('#sou-field-prompt-label').text(`Свой промт для: ${FIELD_LABELS[field] || field}`);
    $('#sou-field-prompt-text').val(localStorage.getItem(FIELD_LS[field]) || '');
    $('#sou-field-prompt-editor').slideDown(120);
  });
  $('#sou-field-prompt-save').on('click', () => {
    const field = String($('#sou-field-prompt-editor').attr('data-active-field') || '');
    if (!field || !FIELD_LS[field]) return;
    const v = String($('#sou-field-prompt-text').val() || '').trim();
  ...
  $('#sou-field-prompt-reset').on('click', () => {
    const field = String($('#sou-field-prompt-editor').attr('data-active-field') || '');
    if (!field || !FIELD_LS[field]) return;
    localStorage.removeItem(FIELD_LS[field]);
    $('#sou-field-prompt-text').val('');
    refreshCustomMarkers();
  ...
  $('#au-quick-ui-theme').on('change', function () { extension_settings[extensionName].uiTheme = $(this).val(); saveSettingsDebounced(); applyUiTheme(); });
  ...
  $('#au-quick-generate').on('click', () => { $('#sea-of-universes-modal-overlay').remove(); onOpenUniverseClick(); });
  $('#au-quick-gallery').on('click', () => { $('#sea-of-universes-modal-overlay').remove(); showGalleryModal(); });
  $('#au-quick-info').on('click', () => { $('#sea-of-universes-modal-overlay').remove(); showWelcomeModal(); });
  $('#au-modal-close').on('click', () => { $('#sea-of-universes-modal-overlay').remove(); });
  $('#sea-of-universes-modal-overlay').on('click', e => { if (e.target === e.currentTarget) $('#sea-of-universes-modal-overlay').remove(); });
}
