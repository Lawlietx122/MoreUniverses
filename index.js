...
// Apply UI theme (light/dark/gray). Accent is always blue (declared in style.css).
function applyUiTheme() {
  const theme = extension_settings[extensionName]?.uiTheme || 'dark';
  // Apply on body (persistent) AND on the open modal root (scoped).
  document.body.setAttribute('data-theme', theme);
  const overlay = document.getElementById('sea-of-universes-modal-overlay');
  if (overlay) overlay.setAttribute('data-theme', theme);
  // Tokens are declared in style.css per [data-theme="..."] selector.
}
...
function showQuickSettings() {
  ...
  const curCreator = extension_settings[extensionName].creatorName || '';
    ...
    <div id="sea-of-universes-modal-overlay" style="${getOverlayStyle()}">
        <div class="au-universal-popup au-quick-popup sou-popup">
            <div class="au-universal-popup-header">
                <div class="au-card-front-header-text"></div>
            ...
            <div class="au-universal-popup-body au-quick-body">
              ...
              <div class="sou-tab-panel sou-tab-panel-active" data-panel="world">
                <div class="sou-grid2">
                  ...
                  <div class="au-quick-row">
                    <label>${t('quick.genre_tags')} <small style="color:#9090b0;">${t('quick.genre_tags_hint')}</small></label>
                    ${genreDropdown}
                  </div>
                  <div class="au-quick-row">
                  ...
                  <div class="au-quick-row" style="grid-column: 1 / -1;">
                    <label>${t('quick.archetype')}</label>
                    ${archDropdown}
                  </div>
                </div>
                <div id="au-quick-custom-wrap" class="sou-field" style="${themeCustomDisplay} padding-top:8px; border-top:0.5px solid var(--border); margin-top:8px;">
                ...
                <div id="au-quick-custom-encounter-wrap" class="sou-field" style="${encounterCustomDisplay} padding-top:8px; border-top:0.5px solid var(--border); margin-top:8px;">
                    <label style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <div id="au-quick-custom-mood-wrap" class="sou-field" style="${moodCustomDisplay} padding-top:8px; border-top:0.5px solid var(--border); margin-top:8px;">
                    <label style="display:flex; justify-content:space-between; align-items:center;">
              ...
              <div class="sou-tab-panel" data-panel="settings">
                ...
                <div class="sou-field" style="padding-top:8px; border-top:0.5px solid var(--border);">
                  <label>Creator (автор карточки)</label>
                  <input type="text" id="au-quick-creator" class="text_pole" value="${esc(curCreator)}" placeholder="Ваш никнейм или псевдоним (необязательно)" style="width:100%;" />
                  <span class="field-hint">Если пусто — поле creator в карточке останется пустым.</span>
                </div>
                <div class="sou-muted-line" style="padding-top:12px; border-top:0.5px solid var(--border); margin-top:12px;">${t('common.results_may_vary')}</div>
              </div>
            </div>
  ...
  const syncMultiChecks = (group, obj, labelFn, settingKey, singleKey, customWrapSelector, fallback = 'random') => {
    ...
    if (customWrapSelector) { values.includes('custom') ? $(customWrapSelector).css('display', 'flex') : $(customWrapSelector).hide(); }
    saveSettingsDebounced();
  };
  $('.au-multi-check[data-group="theme"]').on('change', () => syncMultiChecks('theme', getAllThemes(), getThemeLabel, 'selectedThemes', 'selectedTheme', '#au-quick-custom-wrap'));
  ...
  $('.au-multi-check[data-group="encounter"]').on('change', () => syncMultiChecks('encounter', getAllEncounters(), getEncounterLabel, 'selectedEncounters', 'selectedEncounter', '#au-quick-custom-encounter-wrap'));
  ...
  $('.au-multi-check[data-group="mood"]').on('change', () => syncMultiChecks('mood', getAllMoods(), getMoodLabel, 'selectedMoods', 'selectedMood', '#au-quick-custom-mood-wrap'));
  ...
  $('.au-multi-check[data-group="archetype"]').on('change', () => syncMultiChecks('archetype', getAllArchetypes(), getArchetypeLabel, 'selectedArchetypes', 'selectedArchetype', null, 'none'));
  $('.au-multi-check[data-group="genre"]').on('change', () => { const checks = $(`.au-multi-check[data-group="genre"]`); const values = checks.filter(':checked').map((_, el) => el.value).get().filter(id => getAllGenreTags()[id]); extension_settings[extensionName].selectedGenreTags = values; $(`#au-genre-summary`).text(selectionSummary(values, getGenreTagLabel)); saveSettingsDebounced(); });
  ...
  $('#au-quick-creator').on('input', function () { extension_settings[extensionName].creatorName = String($(this).val() || '').trim(); saveSettingsDebounced(); });
