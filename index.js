...
const RU_UI = {
  ...
  quick: {
    ...
    variant_encounter: 'Встреча варианта {n}',
    variant_mood: 'Настроение варианта {n}',
  ...
  welcome: {
    ...
    contact: 'Форк от <a href="https://t.me/ah_ah_shino4ka" target="_blank" rel="noopener">Шино</a>.',
...
function buildV2CardData(entry) {
  ...
  const tags = ['another-universe', ...themeIds.map(id => `au-theme:${id}`)];
...
function _getSaved(kind) {
  const key = kind === 'theme' ? 'customThemes' : kind === 'encounter' ? 'customEncounters' : 'customMoods';
...
function _deleteSavedItem(kind, id) {
  ...
  for (const selKey of kind === 'theme' ? ['selectedThemes', 'selectedTheme'] : kind === 'encounter' ? ['selectedEncounters', 'selectedEncounter'] : ['selectedMoods', 'selectedMood']) {
...
async function loadSettings() {
  ...
  for (const arrKey of ['customThemes', 'customEncounters', 'customMoods', 'cardTemplates']) {
...
function showQuickSettings() {
  ...
  const allEncounters = getAllEncounters();
  const allMoods = getAllMoods();
  const renderMultiDropdown = (group, obj, selectedIds, labelFn) => {
    const options = sortedEntryKeys(obj, labelFn)
      .map(id => {
        const checked = selectedIds.includes(id) ? 'checked' : '';
        return `<label class="au-multi-option"><input type="checkbox" class="au-multi-check" data-group="${group}" value="${id}" ${checked}> <span>${escapeHtml(labelFn(id))}</span></label>`;
      })
      .join('');
    return `
      <details class="au-multi-select" data-group="${group}">
        ...
        <div class="au-multi-menu">
          ...
          <div class="au-multi-options">${options}</div>
  ...
  const themeDropdown = renderMultiDropdown('theme', allThemes, curThemes, getThemeLabel);
  const encDropdown = renderMultiDropdown('encounter', allEncounters, curEncounters, getEncounterLabel);
  const moodDropdown = renderMultiDropdown('mood', allMoods, curMoods, getMoodLabel);

  // Single-select dropdown (used for per-variant encounter/mood overrides)
  const renderSingleSelect = (id, obj, selectedId, labelFn) => {
    const opts = sortedEntryKeys(obj, labelFn)
      .map(key => `<option value="${key}" ${key === selectedId ? 'selected' : ''}>${escapeHtml(labelFn(key))}</option>`)
      .join('');
    return `<select id="${id}" class="text_pole" style="width:100%;">${opts}</select>`;
  };
  const variant2EncounterSel = renderSingleSelect('au-quick-v2-encounter', allEncounters, extension_settings[extensionName].variant2Encounter || 'random', getEncounterLabel);
  const variant2MoodSel = renderSingleSelect('au-quick-v2-mood', allMoods, extension_settings[extensionName].variant2Mood || 'random', getMoodLabel);
  const variant3EncounterSel = renderSingleSelect('au-quick-v3-encounter', allEncounters, extension_settings[extensionName].variant3Encounter || 'random', getEncounterLabel);
  const variant3MoodSel = renderSingleSelect('au-quick-v3-mood', allMoods, extension_settings[extensionName].variant3Mood || 'random', getMoodLabel);
  ...
  const adaptLangOpts = [
    ...
    .join('');
  const firstMesLangOpts = [
    ['ru', t('quick.first_mes_language_ru')],
    ['en', t('quick.first_mes_language_en')],
    ['auto', t('quick.first_mes_language_auto')],
  ]
    .map(([k, label]) => `<option value="${k}" ${k === curFirstMesLang ? 'selected' : ''}>${label}</option>`)
    .join('');
  const greetingVariantOpts = [1, 2, 3]
    .map(n => `<option value="${n}" ${n === curGreetingVariants ? 'selected' : ''}>${n}</option>`)
    .join('');
  const adaptSizeOpts = [
    ['short', t('adapt.size_short')],
    ['standard', t('adapt.size_standard')],
    ['detailed', t('adapt.size_detailed')],
    ['max', t('adapt.size_max')],
  ]
    .map(([k, label]) => `<option value="${k}" ${k === curAdaptSize ? 'selected' : ''}>${label}</option>`)
    .join('');
  ...
  const escapedAdaptCustomArial = escapeHtml(curAdaptCustomArial);
  const escapedCardTemplate = escapeHtml(curCardTemplate);
  const escapedAntiSlopCard = escapeHtml(curAntiSlopCard);
  const escapedAntiSlopFirstMes = escapeHtml(curAntiSlopFirstMes);
    ...
    <div id="another-universe-modal-overlay" style="${getOverlayStyle()}">
        <div class="au-universal-popup au-quick-popup">
            ...
            <div class="au-universal-popup-body au-quick-body">
                ...
                <div id="au-quick-variant2" class="au-quick-row" style="${variant2Display} flex-direction:column; align-items:stretch; padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                    <label style="display:flex; flex-direction:column; gap:4px;">
                        <small style="color:#9090b0;">${t('quick.variant_encounter', { n: '2' })}</small>
                        ${variant2EncounterSel}
                    </label>
                ...
                <div id="au-quick-variant3" class="au-quick-row" style="${variant3Display} flex-direction:column; align-items:stretch; padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                    <label style="display:flex; flex-direction:column; gap:4px;">
                        <small style="color:#9090b0;">${t('quick.variant_encounter', { n: '3' })}</small>
                        ${variant3EncounterSel}
                    </label>
                    <label style="display:flex; flex-direction:column; gap:4px; margin-top:6px;">
                ...
                <div class="au-quick-row" style="flex-direction:column; align-items:stretch; padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                    ...
                    <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                        <small style="color:#9090b0;">${t('quick.adapt_size')}</small>
                    ...
                    <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                        ...
                        <textarea id="au-quick-card-template" class="text_pole" rows="5" placeholder="${escapeHtml(t('quick.card_template_placeholder'))}" style="width:100%; resize:vertical; min-height:100px;">${escapedCardTemplate}</textarea>
                    ...
                    <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                        ...
                        <textarea id="au-quick-adapt-custom-prompt" class="text_pole" rows="4" placeholder="${escapeHtml(t('adapt.custom_prompt_placeholder'))}" style="width:100%; resize:vertical; min-height:84px;">${escapedAdaptCustomArial}</textarea>
                ...
                <div class="au-quick-row" style="flex-direction:column; align-items:stretch; padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                    <label style="display:flex; flex-direction:column; gap:4px;">
                        ...
                        <textarea id="au-quick-antislop-card" class="text_pole" rows="3" placeholder="${escapeHtml(t('quick.antislop_placeholder'))}" style="width:100%; resize:vertical; min-height:72px; margin-top:4px;">${escapedAntiSlopCard}</textarea>
                    </label>
                    <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                        <span>${t('quick.antislop_firstmes')}</span>
                        <small style="color:#9090b0; line-height:1.35;">${t('quick.antislop_firstmes_hint')}</small>
                        <textarea id="au-quick-antislop-firstmes" class="text_pole" rows="3" placeholder="${escapeHtml(t('quick.antislop_placeholder'))}" style="width:100%; resize:vertical; min-height:72px; margin-top:4px;">${escapedAntiSlopFirstMes}</textarea>
                    </label>
                </div>
  ...
  const renderSavedRow = group => {
    const key = group === 'theme' ? 'customThemes' : group === 'encounter' ? 'customEncounters' : 'customMoods';
    const arr = Array.isArray(extension_settings[extensionName][key]) ? extension_settings[extensionName][key] : [];
    const $row = $(`.au-saved-row[data-group="${group}"]`);
    if (!arr.length) {
      $row.html(`<div class="au-saved-empty">${escapeHtml(t('quick.no_saved_items'))}</div>`);
      return;
    }
    $row.html(
      arr
        .map(
          item =>
            `<div class="au-saved-item" data-id="${item.id}"><span>${escapeHtml(item.label)}</span><span class="au-saved-del" data-group="${group}" data-id="${item.id}" title="${escapeHtml(t('quick.delete_saved'))}">🗑</span></div>`,
        )
        .join(''),
    );
  };
  renderSavedRow('theme');
  renderSavedRow('encounter');
  renderSavedRow('mood');
  ...
  $('.au-multi-search').on('input', function () {
    ...
    $(`.au-multi-check[data-group="${group}"]`)
      .closest('.au-multi-option')
      .each(function () {
  ...
  $(document).off('click.au-saved-del').on('click.au-saved-del', '.au-saved-del', function (e) {
    ...
    // Re-render the dropdown options to remove the deleted item
    const obj = group === 'theme' ? getAllThemes() : group === 'encounter' ? getAllEncounters() : getAllMoods();
    const labelFn = group === 'theme' ? getThemeLabel : group === 'encounter' ? getEncounterLabel : getMoodLabel;
    const selectedIds =
      group === 'theme' ? getSelectedThemes() : group === 'encounter' ? getSelectedEncounters() : getSelectedMoods();
    const opts = sortedEntryKeys(obj, labelFn)
      .map(id => {
        const checked = selectedIds.includes(id) ? 'checked' : '';
        return `<label class="au-multi-option"><input type="checkbox" class="au-multi-check" data-group="${group}" value="${id}" ${checked}> <span>${escapeHtml(labelFn(id))}</span></label>`;
  ...
  // === Save custom theme/encounter/mood ===
  const bindSaveCustom = (btnId, group, getText) => {
    $(btnId).on('click', () => {
      const text = getText();
      if (!text || !text.trim()) {
        toastr.warning(t('quick.custom_empty_warn', { fields: group }), t('quick.custom_empty_title'));
        return;
      }
  ...
  const syncMultiChecks = (group, obj, labelFn, settingKey, singleKey, customWrapSelector, fallback = 'random') => {
    ...
    extension_settings[extensionName][singleKey] = values[0];
    $(`#au-${group}-summary`).text(selectionSummary(values, labelFn));
    if (customWrapSelector) {
      if (values.includes('custom')) $(customWrapSelector).css('display', 'flex');
      else $(customWrapSelector).hide();
    }
    saveSettingsDebounced();
  };
  $('.au-multi-check[data-group="theme"]').on('change', () =>
    syncMultiChecks('theme', getAllThemes(), getThemeLabel, 'selectedThemes', 'selectedTheme', '#au-quick-custom-wrap'),
  );
  ...
  $('.au-multi-check[data-group="encounter"]').on('change', () =>
    syncMultiChecks(
      'encounter',
      getAllEncounters(),
      getEncounterLabel,
      'selectedEncounters',
      'selectedEncounter',
      '#au-quick-custom-encounter-wrap',
    ),
  );
  ...
  $('.au-multi-check[data-group="mood"]').on('change', () =>
    syncMultiChecks('mood', getAllMoods(), getMoodLabel, 'selectedMoods', 'selectedMood', '#au-quick-custom-mood-wrap'),
  );
  ...
  $('#au-quick-first-mes-lang').on('change', function () {
    extension_settings[extensionName].firstMesLanguage = $(this).val();
    saveSettingsDebounced();
  });
  $('#au-quick-greeting-variants').on('change', function () {
    const v = Math.max(1, Math.min(3, parseInt($(this).val(), 10) || 1));
    extension_settings[extensionName].greetingVariants = v;
    saveSettingsDebounced();
    $('#au-quick-variant2').toggle(v >= 2);
    $('#au-quick-variant3').toggle(v >= 3);
  });
  $('#au-quick-v2-encounter').on('change', function () {
    extension_settings[extensionName].variant2Encounter = $(this).val();
    saveSettingsDebounced();
  });
  $('#au-quick-v2-mood').on('change', function () {
    extension_settings[extensionName].variant2Mood = $(this).val();
    saveSettingsDebounced();
  });
  $('#au-quick-v3-encounter').on('change', function () {
    extension_settings[extensionName].variant3Encounter = $(this).val();
    saveSettingsDebounced();
  });
  $('#au-quick-v3-mood').on('change', function () {
    extension_settings[extensionName].variant3Mood = $(this).val();
    saveSettingsDebounced();
  });
  ...
  $('#au-quick-adapt-size').on('change', function () {
    extension_settings[extensionName].adaptSize = $(this).val();
    saveSettingsDebounced();
  });
  $('#au-quick-card-template').on('input', function () {
    extension_settings[extensionName].cardTemplate = String($(this).val() || '').trim();
    saveSettingsDebounced();
  });
  $('#au-quick-adapt-custom-prompt').on('input', function () {
    extension_settings[extensionName].adaptCustomArial = String($(this).val() || '').trim();
    saveSettingsDebounced();
  });
  $('#au-quick-antislop-card').on('input', function () {
    extension_settings[extensionName].antiSlopCard = String($(this).val() || '').trim();
    saveSettingsDebounced();
  });
  $('#au-quick-antislop-firstmes').on('input', function () {
    extension_settings[extensionName].antiSlopFirstMes = String($(this).val() || '').trim();
    saveSettingsDebounced();
  });
  ...
  $('#au-quick-save-template').on('click', () => {
    ...
    if (!Array.isArray(extension_settings[extensionName].cardTemplates)) {
      extension_settings[extensionName].cardTemplates = [];
    }
  ...
  $('#au-quick-load-template').on('click', () => {
    ...
    if (!arr.length) {
      toastr.info(t('quick.no_templates'));
      return;
    }
    let list = arr.map((x, i) => `${i + 1}. ${x.name}`).join('\n');
...
function buildUniverseArial(charName, adaptedCardText, userName, chatContext, options = {}) {
  ...
  if (selectedEncounters.includes('none')) {
    encounterInstruction = '';
  } else if (selectedEncounters.includes('random')) {
    encounterInstruction = 'ENCOUNTER: Choose any type of encounter that feels natural and compelling.';
  } else if (selectedEncounters.includes('custom')) {
    ...
    encounterInstruction = `ENCOUNTER: Combine these selected encounters/mechanics coherently: ${selectedEncounters.map(id => `${getEncounterLabel(id)} (${_allEncounters[id]?.prompt || ''})`).join('; ')}`;
  }
  ...
  if (selectedMoods.includes('none') || selectedMoods.includes('random')) {
    moodInstruction = '';
  } else if (selectedMoods.includes('custom')) {
    ...
    moodInstruction = `MOOD: Blend these selected atmospheres: ${selectedMoods.map(id => `${getMoodLabel(id)} (${_allMoods[id]?.prompt || ''})`).join('; ')}`;
  }
...
<head>
    ...
    <title>🌌 Другая Вселенная — ${escapedCharName}</title>
  ...
  const renderCard = async type => {
    ...
    if (typeof html2canvas !== 'function') {
      try {
        ...
        toastr.error(t('image.cannot_load_html2canvas'), t('common.another_universe'));
      ...
      <div id="au-generation-loading" style="${getOverlayStyle()}; z-index: 999999; background: rgba(0,0,0,0.85); backdrop-filter: blur(10px);">
        <div style="width: min(420px, 92vw); background: rgba(20,18,32,0.96); border: 1px solid rgba(130,160,220,0.25); border-radius: 16px; padding: 24px; box-shadow: 0 20px 60px rgba(0,0,0,0.8);">
          <div style="text-align:center; margin-bottom: 18px;">
            <div style="font-size: 3em; animation: au-spin 2s linear infinite; margin-bottom: 12px;">🌀</div>
            <div style="font-size: 1.15em; font-weight: bold; color: #d0c8e8; letter-spacing: 1px;">${t('loading.opening_portal')}</div>
          </div>
...
async function onOpenUniverseClick() {
  ...
  if (greetingVariants >= 2) {
    const v2e = normalizeSelectionList(extension_settings[extensionName].variant2Encounter || 'random', 'random').filter(id => getAllEncounters()[id]);
    const v2m = normalizeSelectionList(extension_settings[extensionName].variant2Mood || 'random', 'random').filter(id => getAllMoods()[id]);
    variantEncounters.push(v2e.length ? v2e : ['random']);
    variantMoods.push(v2m.length ? v2m : ['random']);
  }
  if (greetingVariants >= 3) {
    const v3e = normalizeSelectionList(extension_settings[extensionName].variant3Encounter || 'random', 'random').filter(id => getAllEncounters()[id]);
    const v3m = normalizeSelectionList(extension_settings[extensionName].variant3Mood || 'random', 'random').filter(id => getAllMoods()[id]);
  ...
  try {
    ...
    if (
      ...
      toastr.error('Модель не адаптировала физиологию {{char}} под выбранную механику. Запустите сборку ещё раз.', t('common.another_universe'));
      return;
    }
