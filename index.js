...
const RU_UI = {
  common: {
    ...
    please_enable: 'Сначала включите расширение «Море Вселенных».',
  ...
  panel: {
    ...
    hint: 'После включения используйте кнопку <b>Море Вселенных</b> в меню чата, чтобы выбрать тему, встречу, настроение и запустить генерацию.',
  },
  quick: {
    title: '',
    subtitle: '',
  ...
  welcome: {
    ...
    subtitle: 'Море миров. Одна история.',
...
const defaultSettings = {
  ...
  uiTheme: 'dark', // 'light' | 'dark' | 'grey'
  accentColor: '#DEB887', // Accent color (CSS var --accent-color)
  gallery: [],
...
async function loadSettings() {
  ...
  if (!['light', 'dark', 'grey'].includes(extension_settings[extensionName].uiTheme)) {
    extension_settings[extensionName].uiTheme = 'dark';
  }
  if (!/^#[0-9a-fA-F]{6}$/.test(extension_settings[extensionName].accentColor || '')) {
    extension_settings[extensionName].accentColor = '#DEB887';
  }
...
// Apply UI theme (light/dark/grey) + accent color via CSS variables on <body>.
function applyUiTheme() {
  const theme = extension_settings[extensionName]?.uiTheme || 'dark';
  const accent = extension_settings[extensionName]?.accentColor || '#DEB887';
  const palettes = {
    dark: { bg: '#11141e', surface: '#1a1d2a', text: '#e8edf2', muted: '#9090b0', border: 'rgba(130,160,220,0.25)' },
    light: { bg: '#f5f6fa', surface: '#ffffff', text: '#1a1d2a', muted: '#5a6478', border: 'rgba(100,120,160,0.25)' },
    grey: { bg: '#2a2d34', surface: '#34373f', text: '#dfe3ea', muted: '#9aa0ad', border: 'rgba(180,190,205,0.25)' },
  };
  const p = palettes[theme] || palettes.dark;
  const body = document.body;
  body.style.setProperty('--accent-color', accent);
  body.style.setProperty('--sou-bg', p.bg);
  body.style.setProperty('--sou-surface', p.surface);
  body.style.setProperty('--sou-text', p.text);
  body.style.setProperty('--sou-muted', p.muted);
  body.style.setProperty('--sou-border', p.border);
  body.setAttribute('data-sou-theme', theme);
}
...
function showQuickSettings() {
  ...
  const curAccent = extension_settings[extensionName].accentColor || '#DEB887';
    ...
    <div id="sea-of-universes-modal-overlay" style="${getOverlayStyle()}">
        <div class="au-universal-popup au-quick-popup sou-popup">
            ...
            <div class="sou-tabs">
                ...
                <button type="button" class="sou-tab" data-tab="gen">⚙️ Генерация</button>
            ...
            <div class="au-universal-popup-body au-quick-body">
              ...
              <div class="sou-tab-panel sou-tab-panel-active" data-panel="world">
                ...
                <div id="au-quick-custom-wrap" class="au-quick-row" style="${themeCustomDisplay} flex-direction:column; align-items:stretch; padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                    ...
                    <small style="color:#7a8aa0; line-height:1.4; margin-top:2px;">${t('quick.custom_theme_hint')}</small>
                ...
                <div class="au-quick-row" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15); display:flex; justify-content:flex-end;">
                  <span id="au-quick-hide-default" class="sou-eye-toggle" title="Скрыть/показать стандартные варианты" style="cursor:pointer; font-size:1.2em; opacity:${hideDef ? '1' : '0.5'};">${hideDef ? '🙈' : '👁️'}</span>
                </div>
                <div class="au-quick-row" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15); flex-direction:column; align-items:stretch;">
                  <label>${t('quick.greeting_variants')}</label>
                  <select id="au-quick-greeting-variants" class="text_pole">${greetingVariantOpts}</select>
                  <small style="color:#9090b0; line-height:1.35;">${t('quick.greeting_variants_hint')}</small>
                </div>
                <div id="au-quick-variant2" class="sou-grid2" style="${variant2Display} padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <div class="au-quick-row" style="flex-direction:column; align-items:stretch;">
                    <small style="color:#9090b0;">${t('quick.variant_encounter', { n: '2' })}</small>
                    ${variant2Enc}
                    <small style="color:#7a8aa0; line-height:1.4; margin-top:3px;">💡 Этот текст вставляется в промт как контекст сцены встречи — не дублируй название из выпадающего списка, а опиши детали обстановки, место, обстоятельства.</small>
                  </div>
                  <div class="au-quick-row" style="flex-direction:column; align-items:stretch;">
                    <small style="color:#9090b0;">${t('quick.variant_mood', { n: '2' })}</small>
                    ${variant2Mood}
                    <small style="color:#7a8aa0; line-height:1.4; margin-top:3px;">💡 Опиши внутреннее состояние, а не просто повтори название из списка. Пример: вместо "грустное" → "персонаж подавлен, потому что..."</small>
                  </div>
                </div>
                <div id="au-quick-variant3" class="sou-grid2" style="${variant3Display} padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <div class="au-quick-row" style="flex-direction:column; align-items:stretch;">
                    <small style="color:#9090b0;">${t('quick.variant_encounter', { n: '3' })}</small>
                    ${variant3Enc}
                    <small style="color:#7a8aa0; line-height:1.4; margin-top:3px;">💡 Этот текст вставляется в промт как контекст сцены встречи — не дублируй название из выпадающего списка, а опиши детали обстановки, место, обстоятельства.</small>
                  </div>
                  <div class="au-quick-row" style="flex-direction:column; align-items:stretch;">
                    <small style="color:#9090b0;">${t('quick.variant_mood', { n: '3' })}</small>
                    ${variant3Mood}
                    <small style="color:#7a8aa0; line-height:1.4; margin-top:3px;">💡 Опиши внутреннее состояние, а не просто повтори название из списка. Пример: вместо "грустное" → "персонаж подавлен, потому что..."</small>
                  </div>
                </div>
              ...
              <div class="sou-tab-panel" data-panel="gen">
                ...
                <div class="au-quick-row" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <label style="font-weight:600;">⚙️ Что генерировать</label>
                  <div class="sou-gen-flags">
                    <label><input type="checkbox" id="au-quick-gen-personality" ${genP ? 'checked' : ''}/> Personality (характер) <span class="sou-custom-marker" data-for="personality"></span> <button type="button" class="sou-field-prompt-btn" data-field="personality">✏️ Свой промт</button></label>
                    <label><input type="checkbox" id="au-quick-gen-scenario" ${genS ? 'checked' : ''}/> Scenario (сценарий) <span class="sou-custom-marker" data-for="scenario"></span> <button type="button" class="sou-field-prompt-btn" data-field="scenario">✏️ Свой промт</button></label>
                    <label><input type="checkbox" id="au-quick-gen-mes" ${genM ? 'checked' : ''}/> Message Example <span class="sou-custom-marker" data-for="mes_example"></span> <button type="button" class="sou-field-prompt-btn" data-field="mes_example">✏️ Свой промт</button></label>
                  </div>
                  <div id="sou-field-prompt-editor" style="display:none; margin-top:8px;">
                    <small style="color:#9090b0;" id="sou-field-prompt-label"></small>
                    ...
                    <div style="display:flex; gap:6px; margin-top:4px;">
                      <button type="button" id="sou-field-prompt-save" class="menu_button" style="flex:1; font-size:0.8em;">Сохранить</button>
                      <button type="button" id="sou-field-prompt-reset" class="menu_button" style="flex:1; font-size:0.8em;">Сбросить</button>
                    </div>
                  </div>
                </div>

                <details class="au-quick-row sou-accordion" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <summary style="cursor:pointer; font-weight:600;">📋 Структура карточки персонажа</summary>
                  <div class="sou-card-structure">${cardStructureHtml}</div>
                </details>

                <details class="au-quick-row sou-accordion" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <summary style="cursor:pointer; font-weight:600;">📝 Шаблон и промпты анкеты</summary>
                  <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                    <small style="color:#9090b0;">${t('quick.card_template')}</small>
                    <textarea id="au-quick-card-template" class="text_pole" rows="5" placeholder="${esc(t('quick.card_template_placeholder'))}" style="width:100%; resize:vertical; min-height:100px;">${esc(curCardTemplate)}</textarea>
                    <small style="color:#9090b0; line-height:1.35;">${t('quick.card_template_hint')}</small>
                    <div style="display:flex; gap:6px; margin-top:6px;">
                      <input id="au-quick-save-template" class="menu_button" type="submit" value="${t('quick.save_template')}" style="flex:1; font-size:0.8em;" />
                      <input id="au-quick-load-template" class="menu_button" type="submit" value="${t('quick.load_template')}" style="flex:1; font-size:0.8em;" />
                    </div>
                  </label>
                  <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                    <small style="color:#9090b0;">${t('quick.adapt_custom_prompt')}</small>
                    ...
                    <small style="color:#9090b0; line-height:1.35;">${t('adapt.custom_prompt_hint')}</small>
                  </label>
                </details>

                <details class="au-quick-row sou-accordion" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <summary style="cursor:pointer; font-weight:600;">🚫 Анти-слоп</summary>
                  <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                    <span>${t('quick.antislop_card')}</span>
                    <small style="color:#9090b0; line-height:1.35;">${t('quick.antislop_card_hint')}</small>
                    <textarea id="au-quick-antislop-card" class="text_pole" rows="3" placeholder="${esc(t('quick.antislop_placeholder'))}" style="width:100%; resize:vertical; min-height:72px; margin-top:4px;">${esc(curAntiSlopCard)}</textarea>
                  </label>
                  <label style="display:flex; flex-direction:column; gap:4px; margin-top:8px;">
                    <span>${t('quick.antislop_firstmes')}</span>
                    <small style="color:#9090b0; line-height:1.35;">${t('quick.antislop_firstmes_hint')}</small>
                    <textarea id="au-quick-antislop-firstmes" class="text_pole" rows="3" placeholder="${esc(t('quick.antislop_placeholder'))}" style="width:100%; resize:vertical; min-height:72px; margin-top:4px;">${esc(curAntiSlopFirstMes)}</textarea>
                  </label>
                </details>
              </div>
              ...
              <div class="sou-tab-panel" data-panel="settings">
                <div class="au-quick-row" style="padding-top:4px;">
                  ...
                  <select id="au-quick-ui-theme" class="text_pole">
                    ...
                    <option value="grey" ${curUiTheme === 'grey' ? 'selected' : ''}>Серая</option>
                  </select>
                </div>
                <div class="au-quick-row" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  <label>Акцентный цвет</label>
                  <div style="display:flex; align-items:center; gap:10px;">
                    <input type="color" id="au-quick-accent-color" value="${esc(curAccent)}" style="width:48px; height:34px; border:none; background:none; cursor:pointer;" />
                    <span id="au-quick-accent-hex" style="font-size:0.85em; color:#9090b0;">${esc(curAccent)}</span>
                    <button type="button" id="au-quick-accent-reset" class="menu_button" style="font-size:0.75em;">Сбросить</button>
                  </div>
                </div>
                <div class="au-quick-row" style="padding-top:8px; border-top:1px dashed rgba(130,100,255,0.15);">
                  ...
                  <small style="color:#9090b0; line-height:1.35;">Если пусто — поле creator в карточке останется пустым.</small>
                </div>
                <div style="font-size:0.75em; color:#9090b0; text-align:center; padding-top:12px; border-top:1px dashed rgba(130,100,255,0.15); margin-top:12px;">${t('common.results_may_vary')}</div>
              </div>
            </div>
            ...
            <style>
              .sou-popup { max-width: 640px !important; }
              .sou-tabs { display:flex; gap:4px; padding:8px 12px 0; border-bottom:1px solid rgba(130,160,220,0.18); flex-shrink:0; }
              .sou-tab { flex:1; padding:8px 6px; background:transparent; border:none; border-bottom:2px solid transparent; color:#9090b0; cursor:pointer; font-size:0.85em; font-weight:600; }
              .sou-tab.sou-tab-active { color: var(--accent-color,#DEB887); border-bottom-color: var(--accent-color,#DEB887); }
              ...
              .sou-sticky-footer { position:sticky; bottom:0; background:rgba(17,20,30,0.98); }
              ...
              .au-optgroup { font-size:0.72em; font-weight:700; color:#8a9ac0; padding:6px 8px 2px; text-transform:uppercase; letter-spacing:0.5px; }
              .au-opt-builtin.sou-hidden-builtin { display:none !important; }
              .sou-card-field { padding:4px 8px; font-size:0.82em; color:#cfd6e6; border-bottom:1px dashed rgba(130,160,220,0.1); }
              .sou-gen-flags { display:flex; flex-direction:column; gap:6px; margin-top:6px; }
              .sou-gen-flags label { display:flex; align-items:center; gap:8px; font-size:0.85em; flex-wrap:wrap; }
              .sou-field-prompt-btn { margin-left:auto; font-size:0.72em; padding:2px 8px; background:rgba(130,160,220,0.12); border:1px solid rgba(130,160,220,0.25); border-radius:6px; color:#cfd6e6; cursor:pointer; }
              .sou-custom-marker { font-size:0.7em; color:var(--accent-color,#DEB887); }
              .sou-combo { position:relative; }
              .sou-combo-list { max-height:180px; overflow:auto; margin-top:4px; border:1px solid rgba(130,160,220,0.3); border-radius:8px; background:rgba(12,14,28,0.96); display:none; }
              .sou-combo.open .sou-combo-list { display:block; }
              .sou-combo-option { padding:7px 10px; cursor:pointer; font-size:0.85em; color:#e8edf2; display:flex; justify-content:space-between; align-items:center; }
              .sou-combo-option:hover { background:rgba(130,160,220,0.14); }
              .sou-combo-option.sou-combo-active { background:rgba(222,184,135,0.18); }
              .sou-combo-del { color:#ff8888; opacity:0.6; cursor:pointer; }
              .sou-combo-del:hover { opacity:1; }
              .sou-combo-custom { color:var(--accent-color,#DEB887); font-style:italic; }
              .sou-accordion { padding:8px 0; }
              .sou-accordion summary { list-style:none; }
              .sou-accordion summary::-webkit-details-marker { display:none; }
              .au-multi-select { width: 100%; position: relative; }
              .au-multi-select summary { list-style: none; cursor: pointer; min-height: 34px; max-height: 74px; overflow: hidden; padding: 8px 32px 8px 10px; border: 1px solid rgba(130,160,220,0.35); border-radius: 8px; background: rgba(10,12,24,0.55); color: #e8edf2; line-height: 1.35; }
              .au-multi-select summary::-webkit-details-marker { display: none; }
              .au-multi-select summary:after { content: '▾'; position: absolute; right: 12px; top: 8px; color: #aab7d6; }
              .au-multi-select[open] summary:after { content: '▴'; }
              .au-multi-menu { margin-top: 6px; max-height: 280px; overflow: auto; padding: 8px; border: 1px solid rgba(130,160,220,0.35); border-radius: 8px; background: rgba(12,14,28,0.96); box-shadow: 0 12px 28px rgba(0,0,0,0.35); }
              .au-multi-options { max-height: 180px; overflow: auto; }
              .au-multi-option { display: flex; gap: 8px; align-items: center; padding: 7px 8px; border-radius: 6px; color: #e8edf2; cursor: pointer; }
              .au-multi-option:hover { background: rgba(130,160,220,0.14); }
              .au-multi-option input { flex: 0 0 auto; }
              .au-multi-search { font-size: 0.85em; }
              .au-saved-item { display:flex; justify-content:space-between; align-items:center; gap:6px; padding:4px 6px; border-radius:6px; font-size:0.8em; color:#cfd6e6; }
              .au-saved-item:hover { background: rgba(130,160,220,0.12); }
              .au-saved-del { cursor:pointer; color:#ff8888; opacity:0.6; }
              .au-saved-del:hover { opacity:1; }
              .au-saved-empty { font-size:0.75em; color:#8090a8; font-style:italic; padding:2px 6px; }
            </style>
        </div>
    </div>`;
  ...
  function refreshCustomMarkers() {
    for (const [field, lsKey] of Object.entries(FIELD_LS)) {
      ...
      $(`.sou-custom-marker[data-for="${field}"]`).html(has ? '🌟 кастомный промт активен' : '');
    }
  }
  ...
  $('#au-quick-hide-default').on('click', function () {
    ...
    saveSettingsDebounced();
    $(this).text(v ? '🙈' : '👁️').css('opacity', v ? '1' : '0.5');
    applyHideDefault(v);
  });
  ...
  $('#au-quick-card-template').on('input', function () { extension_settings[extensionName].cardTemplate = String($(this).val() || '').trim(); saveSettingsDebounced(); });
  ...
  $('#au-quick-antislop-card').on('input', function () { extension_settings[extensionName].antiSlopCard = String($(this).val() || '').trim(); saveSettingsDebounced(); });
  $('#au-quick-antislop-firstmes').on('input', function () { extension_settings[extensionName].antiSlopFirstMes = String($(this).val() || '').trim(); saveSettingsDebounced(); });
  ...
  $('.sou-field-prompt-btn').on('click', function () {
    ...
    $('#sou-field-prompt-label').text(`Свой промт для: ${FIELD_LABELS[activeField]}`);
    $('#sou-field-prompt-text').val(localStorage.getItem(FIELD_LS[activeField]) || '');
    $('#sou-field-prompt-editor').slideDown(120);
  });
  $('#sou-field-prompt-save').on('click', () => {
    if (!activeField) return;
    const v = String($('#sou-field-prompt-text').val() || '').trim();
    if (v) localStorage.setItem(FIELD_LS[activeField], v); else localStorage.removeItem(FIELD_LS[activeField]);
    refreshCustomMarkers();
    toastr.success(t('quick.template_saved'));
  });
  $('#sou-field-prompt-reset').on('click', () => {
    if (!activeField) return;
    localStorage.removeItem(FIELD_LS[activeField]);
    $('#sou-field-prompt-text').val('');
    refreshCustomMarkers();
    toastr.success(t('quick.template_deleted'));
  });

  // === Card template save / load ===
  $('#au-quick-save-template').on('click', () => {
    const name = prompt(t('quick.template_name_prompt'));
    if (!name || !name.trim()) return;
    const tpl = String($('#au-quick-card-template').val() || '').trim();
    const anti1 = String($('#au-quick-antislop-card').val() || '').trim();
    const anti2 = String($('#au-quick-antislop-firstmes').val() || '').trim();
    if (!Array.isArray(extension_settings[extensionName].cardTemplates)) extension_settings[extensionName].cardTemplates = [];
    extension_settings[extensionName].cardTemplates.push({ name: name.trim().slice(0, 80), template: tpl, antiSlopCard: anti1, antiSlopFirstMes: anti2 });
    saveSettingsDebounced();
    toastr.success(t('quick.template_saved'));
  });
  $('#au-quick-load-template').on('click', () => {
    const arr = Array.isArray(extension_settings[extensionName].cardTemplates) ? extension_settings[extensionName].cardTemplates : [];
    if (!arr.length) { toastr.info(t('quick.no_templates')); return; }
    const list = arr.map((x, i) => `${i + 1}. ${x.name}`).join('\n');
    const choice = prompt(`${t('quick.load_template')}\n\n${list}\n\nНомер:`);
    const idx = parseInt(choice, 10) - 1;
    if (isNaN(idx) || idx < 0 || idx >= arr.length) return;
    const tpl = arr[idx];
    $('#au-quick-card-template').val(tpl.template || '');
    $('#au-quick-antislop-card').val(tpl.antiSlopCard || '');
    $('#au-quick-antislop-firstmes').val(tpl.antiSlopFirstMes || '');
    ...
    saveSettingsDebounced();
    toastr.success(t('quick.template_loaded'));
  });
  ...
  $('#au-quick-accent-color').on('input', function () { const v = $(this).val(); extension_settings[extensionName].accentColor = v; saveSettingsDebounced(); $('#au-quick-accent-hex').text(v); applyUiTheme(); });
  $('#au-quick-accent-reset').on('click', () => { extension_settings[extensionName].accentColor = '#DEB887'; saveSettingsDebounced(); $('#au-quick-accent-color').val('#DEB887'); $('#au-quick-accent-hex').text('#DEB887'); applyUiTheme(); });
    ...
    <div id="au-welcome-overlay" style="${getOverlayStyle()}">
        <div class="au-universal-popup">
            ...
            <div class="au-universal-popup-body" style="padding: 24px; text-align: left;">
                <p style="font-size:0.95em; line-height:1.6; margin:0 0 16px 0; color:#e8edf2;">${t('welcome.description')}</p>

                <div style="margin-top: 24px; font-size: 0.7em; color:#a0aac0; line-height:1.55; padding-top: 14px; border-top: 1px dashed rgba(130, 160, 220, 0.2);">
                    ${t('welcome.credits')}
                </div>

                <div style="margin-top: 10px; font-size: 0.75em; color:#c0c8d8; font-weight:600;">
                    ${t('welcome.author')}
                </div>
            </div>
